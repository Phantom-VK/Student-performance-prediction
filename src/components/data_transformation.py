...
class DataTransformation:
    ...
    def get_data_transformer_object(self) -> ColumnTransformer:
        """
        Creates a pipeline to transform dataset columns
        :return: Column transformer preprocessor
        """
        ...
        try:
            ...
            cat_pipeline = Pipeline(
                steps=[
                    ...
                    ("onehotencder", OneHotEncoder(handle_unknown='ignore'))
                ]
            )
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            # It combines arrays horizontally (side-by-side)
            train_arr = np.c_[
                input_feature_train_processed_arr,
                np.array(target_feature_train_df)  # Converts target Series to NumPy array
            ]
            test_arr = np.c_[
                input_feature_test_processed_arr,
                np.array(target_feature_test_df)  # Converts target Series to NumPy array
            ]
            logging.info("Processed train and test dataset.")
